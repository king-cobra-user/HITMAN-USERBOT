...
telegraph = Telegraph()
r = telegraph.create_account(short_name="HitmanUserBot")
auth_url = r["auth_url"]
...
async def picgen(client, message):
    ...
    if response.status_code == 200:
        with open("HITMAN.jpg", "wb") as f:
            f.write(response.content)
    captin = f"Fake Image By Hitman.\nGet Your Own Friday From @Hitmanuserbotchat."
    fole = "FRIDAYOT.jpg"
    if message.reply_to_message:
        await client.send_photo(
            message.chat.id,
...
async def adityalogo(client, message):
    ...
    if message.reply_to_message:
        ...
        await client.send_photo(
            message.chat.id, photo=file_name, caption="Made Using HITMANUserBot"
        )
    await client.send_chat_action(message.chat.id, "cancel")
    await event.delete()
    if os.path.exists(file_name):
