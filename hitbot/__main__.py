...
from hitmanbot.hitmanbotConfig import Var
from hitmanbot.utils import load_module, start_mybot, load_pmbot
from pathlib import Path
import telethon.utils
from hitmambot import CMD_HNDLR
...
print("HitmanBot has been deployed! ")

print("Setting up TGBot")
path = "hitmanbot/plugins/mybot/*.py"
files = glob.glob(path)
for name in files:
    with open(name) as f:
        path1 = Path(f.name)
...
if LOAD_MYBOT == "True":
    path = "hitmanbot/plugins/mybot/pmbot/*.py"
    files = glob.glob(path)
    for name in files:
        with open(name) as f:
            path1 = Path(f.name)
...
print("HitmanBot has been fully deployed! Do Visit @Hitmanuserbotchat")
